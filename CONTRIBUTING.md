# Contributing to mcp-gateway-lite
...
- **Zero new runtime dependencies** without an issue discussing why first. The dependency-free constraint is a feature: this is stdlib-only Go, no frameworks, no third-party packages.
...
CI runs that same command plus `gofmt`/`go vet` checks, a build, and an end-to-end smoke test (`ci/smoke.sh`) against a real compiled binary and a stub upstream; green CI is required, no exceptions (including for maintainers — check the history: it's how the whole repo was built).
